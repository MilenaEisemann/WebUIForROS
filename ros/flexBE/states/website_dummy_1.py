...
class WebsiteDummyState1(EventState):
    '''
	...
	self.client_website_proxy = rospy.ServiceProxy(self.website_service_proxy, WebServiceProxyMsg)
        #self.client_website_proxy = ProxyServiceCaller({self.website_service_proxy: BehaviorControlMsg})
	...
	if self._response.success == True and (rospy.Time.now() - self._start_time).to_sec() > 10:
	    Logger.loginfo(self._response.success)
            return 'done'
	...
	service_call.command = "home"
	#resp1 = self.client_website_proxy(service_call.command)
        #service_call.command = str("home")
        #service_call.wait_for_execution = self._wait_for_execution
	#self._response = self.client_website_proxy.call(self.website_service_proxy, "home")
